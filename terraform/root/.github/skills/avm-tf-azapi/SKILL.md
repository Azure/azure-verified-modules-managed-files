...
`hashicorp/azurerm ~> 4.0` is permitted only when required for a data-plane or other non-ARM operation that genuinely cannot be implemented with those AzAPI resource forms. Each `azurerm_*` resource or data-source block independently scopes to one specific unsupported operation, adds the prescribed `avm_provider_azurerm_disallowed` rule override to the narrowest supported TFLint override file, documents the exact block and why AzAPI cannot implement it with an upstream AzAPI issue or pull request, and is replaced when support ships. Never use an inline TFLint suppression. One valid block does not authorize another. Do not use the exception for any control-plane resource. Follow `avm-tf-tflint` for override scope and precedence.
