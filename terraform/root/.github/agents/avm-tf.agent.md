...
For every RMFR4 or RMFR5 interface migration, fetch `docs/content/specs-defs/specs/terraform/interfaces.md` and the matching `docs/static/includes/interfaces/tf/int.<interface>.schema.tf` from `Azure/Azure-Verified-Modules`. Treat the entire canonical variable declaration as atomic: copy it in full, replace only documented placeholders, and preserve attribute ordering, types, defaults, nullability, descriptions, validations, and error messages exactly. Lint notices are migration hints, not complete implementation instructions.
